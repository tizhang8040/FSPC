...
def getParameters(_parameters={}):
    ...
    parameters['heatMode'] = "sticking"	#contact/sticking
    ...
    if parameters['heatMode'] == "sticking":
        # sticking-FeTopAl
        parameters['stickKMech_FeTopAl'] = 1.e5  #????
        parameters['stickKTher_FeTopAl'] = 1.e5  #????
        # sticking-AlFeBot
        parameters['stickKMech_AlFeBot'] = 1.e5  #????
        parameters['stickKTher_AlFeBot'] = 1.e5  #????
    ...
    parameters['dtMax2'] = 0.5  # maximal time step (sec)
    parameters['dtMax3'] = 0.5  # maximal time step (sec)
    parameters['dtMax4'] = 0.5  # maximal time step (sec)
    ...
    parameters['metaforStandalone'] = False
...
def buildDomain(_parameters={}):
    ...
    if parameters['heatMode'] == "contact":
        ...
        # sticking-AlFeBot
        stickKMech_AlFeBot = parameters['stickKMech_AlFeBot']
        stickEleType_AlFeBot = TmSticking3DElement
        stickKTher_AlFeBot = parameters['stickKTher_AlFeBot']
...
###----------------------------------------------------------------------------------------------------------###
    ...
    if parameters['heatMode'] == "contact":
        ...
        defineTmStickElePropInter(stickEleType_FeTopAl, idMatSI_FeTopAl, STIFF_ANALYTIC, SA_ONCE, stickKTher_FeTopAl, interset, idMatSI_FeTopAl, groups['AlS_Up'], groups['FeTop_Down'])
        # sticking-Al/FeBot
        cstSpringMater(materset, idMatSI_AlFeBot, stickKMech_AlFeBot)
        defineTmStickElePropInter(stickEleType_AlFeBot, idMatSI_AlFeBot, STIFF_ANALYTIC, SA_ONCE, stickKTher_AlFeBot, interset, idMatSI_AlFeBot, groups['FeBot_Up'], groups['AlS_Down'])
    ...
    if parameters['metaforStandalone']:
        # time step manager
    ...
    # parameters for FSPC
    if not parameters['metaforStandalone']:
