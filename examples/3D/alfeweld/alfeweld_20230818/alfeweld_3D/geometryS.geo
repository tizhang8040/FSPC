...
/* #####################################
   ...
   ##################################### */
hmax_steel              = 3.0;
hmax_steel_tool         = 0.75;
hmax_steel_FSI          = 0.60;

hmax_backingPlate       = 4.0;
hmax_backingPlate_top   = 2.5;
...
Point(1003) = {L+LR,  -HB + HB, -WL , hmax_backingPlate_top}; 
Point(1004) = {-LL ,  -HB + HB, -WL , hmax_backingPlate_top};
Point(1005) = {-LL ,  -HB     , WR+W, hmax_backingPlate};
Point(1006) = {L+LR,  -HB     , WR+W, hmax_backingPlate};
Point(1007) = {L+LR,  -HB + HB, WR+W, hmax_backingPlate_top};
Point(1008) = {-LL ,  -HB + HB, WR+W, hmax_backingPlate_top};


// Points Steel IN  ==================
Point(1009) = {-LL/2     ,   -HB       , 0    , hmax_backingPlate};
Point(1010) = {-LL/2     ,   -HB       , WCold, hmax_backingPlate};
Point(1011) = {L+LR-LL/2 ,   -HB       , WCold, hmax_backingPlate};
Point(1012) = {L+LR-LL/2 ,   -HB       , 0    , hmax_backingPlate};
...
// lines Steel IN ====================
Line(1013) = {1010, 1009}; Line(1014) = {1009, 1012};
Line(1015) = {1012, 1011}; Line(1016) = {1011, 1010};
...
Curve Loop(1007) = {1013, 1014, 1015, 1016};
...
// volume ===============
Surface Loop(2) = {1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008};
Volume(2) = {2};   
...
Physical Surface("Clamped") = {1,2,3,5,6,2001,2002,2003,2004,1001,1002,1003,1004,1005};
Physical Surface("FSInterface") = {7,1008,2005,2006,2007,2008};
Physical Surface("Bottom") = {1007};
Physical Volume("Solid_steel") = {1};
Physical Volume("Solid_aluminium") = {3};
Physical Volume("Solid_backingPlate") = {2};

//Transfinite Line {2,4,6,8} = 3 Using Progression 1;
//Transfinite Line {2001,2002,2003,2004} = 3 Using Progression 1;