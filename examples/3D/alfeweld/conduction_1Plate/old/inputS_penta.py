...
def getMetafor(parm):
    ...
    # check mesh
    if 0 : #metaforStandalone and vizMesh:
        win = w.VizWin()
        #win.add(pointset)
        #win.add(curveset)
        #win.add(volumeset)
        ...
        win.add(geometry.getMesh().getVolumeSet())
        #win.add(geometry.getSurfaceSet())  # rigid tool
    ...
    app11 = w.FieldApplicator(11)
    app11.push(groups['FeTop'])
    interactionset.add(app11)
    
    prp11 = w.ElementProperties(w.TmPentaVolume3DElement)
    prp11.put(w.CAUCHYMECHVOLINTMETH,w.VES_CMVIM_SRIPR)
    prp11.put(w.STIFFMETHOD,w.STIFF_ANALYTIC)
    # ===== tianyu =======
    prp11.put(w.DAMPSTIFF, dampStiff)
    prp11.put(w.DAMPMASS, dampMass)
    
    # ====================
    #prp1.put(w.GRAVITY_Z,-9.81)
    prp11.put(w.MATERIAL,1)
    app11.addProperty(prp11)
    ...
    ci = defineTmRDContactElePropInter(eleType_CI, idMatCI, w.STIFF_ANALYTIC, w.AIC_ONCE, T_Tool, fctT_Tool, interactionset, idMatCI, surf_RT, groups['FeTopUp'], setSN_CI)
    # ====================
...
#    interactionset.add(inter)
    ...
    AmplDispX_RT = 80/10                    # horizontal displacement
    ...
    AmplDispY_RT = -HS*0.05 #1.e-1            # vertical displacement
    ...
    initcondset.define(groups['FeTop'],w.Field1D(w.TO,w.AB),0.0)
    initcondset.define(groups['FeTop'],w.Field1D(w.TO,w.RE),300)
    ...
    ti_M = w.DampedAlphaGeneralizedTimeIntegration(metafor)
