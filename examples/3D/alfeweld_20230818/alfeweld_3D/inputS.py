...
def getMetafor(parm):
    ...
    W = 20                      #   solid domain width
    
    rExt_RT = 12                #   rigid tool radius
    ...
    T_Tool = 1800.                      #   Temperature of the tool 
    setSN_CI = True #False              #   
    #time step manager ------------------   
    time0 = 0.0                         #   initial time
    time1 = time0+10.0                  #   vertical displacement time (tool)
    time2 = time1+5.4                   #   horizontal displacement time (tool)
    ...
    app.push(groups['Solid_steel'])
    interactionset.add(app)

    app2 = w.FieldApplicator(2)
    app2.push(groups['Solid_aluminium'])
    interactionset.add(app2)

    app3 = w.FieldApplicator(3)
    app3.push(groups['Solid_backingPlate'])
    ...
    materset(2).put(w.ELASTIC_MODULUS,2e5)
    materset(2).put(w.THERM_EXPANSION,0)
    materset(2).put(w.HEAT_CAPACITY,900e6)
    materset(2).put(w.MASS_DENSITY,2.71e-9)
    materset(2).put(w.POISSON_RATIO,0)
    materset(2).put(w.CONDUCTIVITY,200)
    materset(2).put(w.DISSIP_TE,0)
    materset(2).put(w.DISSIP_TQ,0)

    materset.define(3,w.TmElastHypoMaterial)
    materset(3).put(w.ELASTIC_MODULUS,2e5)
    materset(3).put(w.THERM_EXPANSION,0)
    materset(3).put(w.HEAT_CAPACITY,500e6)
    materset(3).put(w.MASS_DENSITY,8.00e-9)
    materset(3).put(w.POISSON_RATIO,0)
    materset(3).put(w.CONDUCTIVITY,16)
    ...
    ci = defineTmRDContactElePropInter(eleType_CI, idMatCI, w.STIFF_ANALYTIC, w.AIC_ONCE, T_Tool, fctT_Tool, interactionset, idMatCI, surf_RT, groups['Solid_steel'], setSN_CI)
    # ====================

    # Elements for surface heat flux

    prp4 = w.ElementProperties(w.NodTriangleHeatFlux3DElement)
    ...
    initcondset.define(groups['Solid_steel'],w.Field1D(w.TO,w.AB),300)
    initcondset.define(groups['Solid_aluminium'],w.Field1D(w.TO,w.AB),300)
    initcondset.define(groups['Solid_backingPlate'],w.Field1D(w.TO,w.AB),300)
