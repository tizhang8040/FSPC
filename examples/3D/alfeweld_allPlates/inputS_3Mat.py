...
def getMetafor(parm):
    ...
    rExt_RT = 12.275/1          #   rigid tool radius
    ...
    app1.push(groups['FeTop'])
    interactionset.add(app1)
    
    app2 = w.FieldApplicator(2)
    app2.push(groups['FeBot'])
    ...
    materset(2).put(w.ELASTIC_MODULUS,193.e3)
    materset(2).put(w.THERM_EXPANSION,0)
    materset(2).put(w.HEAT_CAPACITY,500.0e6)
    materset(2).put(w.MASS_DENSITY,8.00e-9)
    materset(2).put(w.CONDUCTIVITY,16.3)
    materset(2).put(w.POISSON_RATIO,0.27)
    materset(2).put(w.DISSIP_TE,0)
    materset(2).put(w.DISSIP_TQ,0)
    
    materset.define(3,w.TmElastHypoMaterial)
    materset(3).put(w.ELASTIC_MODULUS,69.e3)
    materset(3).put(w.THERM_EXPANSION,0)
    materset(3).put(w.HEAT_CAPACITY,900.0e6)
    materset(3).put(w.MASS_DENSITY,2.71e-9)
    materset(3).put(w.CONDUCTIVITY,229.0)
    materset(3).put(w.POISSON_RATIO,0.33)
    ...
    ci = defineTmRDContactElePropInter(eleType_CI, idMatCI, w.STIFF_ANALYTIC, w.AIC_ONCE, T_Tool, fctT_Tool, interactionset, idMatCI, surf_RT, groups['Top'], setSN_CI)
    # ====================

    # Elements for surface heat flux

    prp100 = w.ElementProperties(w.NodTriangleHeatFlux3DElement)
    heat = w.NodInteraction(100)
    heat.push(groups['FSInterface'])
    heat.addProperty(prp100)
    ...
    initcondset.define(groups['FeBot'],w.Field1D(w.TO,w.AB),300)
    initcondset.define(groups['Al'],w.Field1D(w.TO,w.AB),300)
